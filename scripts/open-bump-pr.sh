...
#                     from facebook.cs check-min-os; a raised floor is breaking for
...
WRAPPER_CSPROJ="src/Maui.Facebook.Core/Maui.Facebook.Core.csproj"
case "$PLATFORM" in
  android)
    PLATFORM_NAME="Android"; WORKFLOW="TryBumpAndroid"; OS_NAME="Android API level"
    BINDING_DIR="src/Maui.Facebook.Core.Android"; TFM="net10.0-android"
    SOURCE_URL="https://central.sonatype.com/artifact/com.facebook.android/facebook-core/${TARGET}"
  ...
  ios)
    ...
    BINDING_DIR="src/Maui.Facebook.Core.iOS"; TFM="net10.0-ios"
    SOURCE_URL="https://github.com/facebook/facebook-ios-sdk/releases/tag/v${TARGET}"
...
TITLE="chore(deps): bump Facebook ${PLATFORM_NAME} SDK to ${TARGET}"
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  git config user.name  "facebook-bump-bot[bot]"
  git config user.email "facebook-bump-bot[bot]@users.noreply.github.com"
...
if [[ "$STATUS" == "green" ]]; then
  ...
  STATUS_LINE="🛑 **DRAFT - upstream marks this release as breaking** (a new major version, or a \`Removed\`/breaking note in its changelog). Everything builds and the binding's own minimum OS version is unchanged, but read the upstream note above before merging: the break may be an API removal the wrapper does not call, or a support-policy change invisible in the artifact. Mark the PR ready once you have decided it is safe for consumers."
  BUILD_LINE="✅ Binding builds (\`dotnet build -c Release\`)"
  WRAPPER_LINE="✅ Wrapper compiles against the packed binding (\`${TFM}\`, wrapper csproj untouched)"
else
...
if [[ -n "$CHANGELOG_EXCERPT" ]]; then
  ...
  EXCERPT_BLOCK="_No notes published yet - \`facebook/facebook-ios-sdk\` CHANGELOG.md has no entry for this version._"
else
  EXCERPT_BLOCK="_No notes published yet - \`facebook/facebook-android-sdk\` CHANGELOG.md has no entry for this version._"
fi
...
gh label create upstream-breaking --color D93F0B --description "Upstream marks this native SDK release as breaking (new major or a Removed note)" --force >/dev/null
...
# Meta can ship 18.3.1 while the 18.3.0 PR is still open. Both are opened against the
...
if [[ "$STATUS" == "green" ]]; then
  gh pr list --state open --base "$BASE_BRANCH" --json number,headRefName,isDraft \
    ...
    | while read -r old_num old_branch old_draft; do
        ...
        if [[ "$(dotnet run scripts/facebook.cs -- compare-versions "$old_version" "$TARGET")" != "-1" ]]; then
        ...
        gh pr close "$old_num" --comment "Superseded by ${URL} - Facebook ${PLATFORM_NAME} \`${TARGET}\` is now the newest release upstream, so this bump to \`${old_version}\` will not be merged. ${delete_note}" \
